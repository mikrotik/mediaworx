 ...
 * @return mixed
 */
function _echelon_upload_error($error){
...
{
    if(isset($_FILES['file']) && _echelon_upload_error($_FILES['file']['error'])){
        header('HTTP/1.0 400 Bad error');
        echo _echelon_upload_error($_FILES['file']['error']);
        die;
    }
...
function handle_lead_attachments($leadid,$index_name = 'file', $form_activity = false)
{
   ...
   if(isset($_FILES[$index_name]) && empty($_FILES[$index_name]['name']) && $form_activity){return;}
    ...
    if(isset($_FILES[$index_name]) && _echelon_upload_error($_FILES[$index_name]['error'])){
        header('HTTP/1.0 400 Bad error');
        echo _echelon_upload_error($_FILES[$index_name]['error']);
        die;
    }
...
function handle_company_logo_upload()
{
   ...
   if(isset($_FILES['company_logo']) && _echelon_upload_error($_FILES['company_logo']['error'])){
        set_alert('warning',_echelon_upload_error($_FILES['company_logo']['error']));
        return false;
    }
